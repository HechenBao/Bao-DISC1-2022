%v_ct=1:1:size(zscore_ct_IC,1);
%C_ct=nchoosek(v_ct,3);

for k=1:50
    r = randperm(size(zscore_ct_IC,1));
    r = r(1:round(size(zscore_ct_IC,1).*0.75));
for i=1:size(r,2)
    rand_ct_IC(i,:) = zscore_ct_IC(r(i),:);
    rand_ct_MD(i,:) = zscore_ct_MD(r(i),:);
end
    mean_rand_ct_IC(k,:)=mean(rand_ct_IC,1);
    mean_rand_ct_MD(k,:)=mean(rand_ct_MD,1);
    
    lags=[-4:0.1:4]';
    
    R_xcorr_ct(k,:)=xcorr(mean_rand_ct_MD(k,11:51),mean_rand_ct_IC(k,11:51),'coeff');
    [R_ct(k,:),P_ct(k,:)]=corr(mean_rand_ct_MD(k,11:51)',mean_rand_ct_IC(k,11:51)');
    [Peak_ct(k,:),MaxInd_ct(k,:)]= max(abs(R_xcorr_ct(k,:)));
end

for k=1:50
    r = randperm(size(zscore_disc1_IC,1));
    r = r(1:round(size(zscore_disc1_IC,1).*0.75));
for i=1:size(r,2)
    rand_disc1_IC(i,:) = zscore_disc1_IC(r(i),:);
    rand_disc1_MD(i,:) = zscore_disc1_MD(r(i),:);
end
    mean_rand_disc1_IC(k,:)=mean(rand_disc1_IC,1);
    mean_rand_disc1_MD(k,:)=mean(rand_disc1_MD,1);
    
    lags=[-4:0.1:4]';
    
    R_xcorr_disc1(k,:)=xcorr(mean_rand_disc1_MD(k,11:51),mean_rand_disc1_IC(k,11:51),'coeff');
    [R_disc1(k,:),P_disc1(k,:)]=corr(mean_rand_disc1_MD(k,11:51)',mean_rand_disc1_IC(k,11:51)');
    [Peak_disc1(k,:),MaxInd_disc1(k,:)]= max(abs(R_xcorr_disc1(k,:)));
end

%% output
output_mean_ct=[mean(mean_rand_ct_IC,1);mean(mean_rand_ct_MD,1)];
output_mean_disc1=[mean(mean_rand_disc1_IC,1);mean(mean_rand_disc1_MD,1)];
