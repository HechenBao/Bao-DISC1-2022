%v_ct=1:1:size(zscore_ct_CA1,1);
%C_ct=nchoosek(v_ct,3);

for k=1:50
    r = randperm(size(zscore_ct_CA1,1));
    r = r(1:round(size(zscore_ct_CA1,1).*0.75));
for i=1:size(r,2)
    rand_ct_CA1(i,:) = zscore_ct_CA1(r(i),:);
    rand_ct_CA3(i,:) = zscore_ct_CA3(r(i),:);
end
    mean_rand_ct_CA1(k,:)=mean(rand_ct_CA1,1);
    mean_rand_ct_CA3(k,:)=mean(rand_ct_CA3,1);
    
    lags=[-4:0.1:4]';
    
    R_xcorr_ct(k,:)=xcorr(mean_rand_ct_CA3(k,11:51),mean_rand_ct_CA1(k,11:51),'coeff');
    [R_ct(k,:),P_ct(k,:)]=corr(mean_rand_ct_CA3(k,11:51)',mean_rand_ct_CA1(k,11:51)');
    [Peak_ct(k,:),MaxInd_ct(k,:)]= max(abs(R_xcorr_ct(k,:)));
end

for k=1:50
    r = randperm(size(zscore_disc1_CA1,1));
    r = r(1:round(size(zscore_disc1_CA1,1).*0.75));
for i=1:size(r,2)
    rand_disc1_CA1(i,:) = zscore_disc1_CA1(r(i),:);
    rand_disc1_CA3(i,:) = zscore_disc1_CA3(r(i),:);
end
    mean_rand_disc1_CA1(k,:)=mean(rand_disc1_CA1,1);
    mean_rand_disc1_CA3(k,:)=mean(rand_disc1_CA3,1);
    
    lags=[-4:0.1:4]';
    
    R_xcorr_disc1(k,:)=xcorr(mean_rand_disc1_CA3(k,11:51),mean_rand_disc1_CA1(k,11:51),'coeff');
    [R_disc1(k,:),P_disc1(k,:)]=corr(mean_rand_disc1_CA3(k,11:51)',mean_rand_disc1_CA1(k,11:51)');
    [Peak_disc1(k,:),MaxInd_disc1(k,:)]= max(abs(R_xcorr_disc1(k,:)));
end
%% output
output_mean_ct=[mean(mean_rand_ct_CA1,1);mean(mean_rand_ct_CA3,1)];
output_mean_disc1=[mean(mean_rand_disc1_CA1,1);mean(mean_rand_disc1_CA3,1)];
...
colorbar;
subplot(2,1,2)
imagesc(R_xcorr_disc1)
caxis([-1 1]);
colorbar;

%% export plot 4 (with lag 0, xcorr-lags)
figure;
x0=0;
y0=0;
width=1000;
...
saveas(gcf,'CA-fam.tiff')
%saveas(gcf,'CA-test.tiff')
