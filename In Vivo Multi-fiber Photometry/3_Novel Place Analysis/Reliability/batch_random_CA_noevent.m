...
for j = [1:length(Data)]
    ...
    for i = 1:size(r,2)
        raster_random_CA1(i,:) = CA1_ratio(r(i)-before:r(i)+after);
    end
    for i = 1:size(r,2)
    ...
    for i=1:size(r,2)
        zscore_rand_CA1(i,:) = zscore(raster_random_CA1(i,:));
    end
    for i=1:size(r,2)
    ...
    %% CA1CA3
    % hab save
%     savdir = 'D:\disc1_photometry_data\disc1_multi-fiber\Revision data\revision codes\np\reliability\CA1CA3\rand events\CA1CA3 hab rand\Output';
%     save(fullfile(savdir,[varName '_rand.mat']),'zscore_rand_CA1','zscore_rand_CA3') % save for individual animal
%     
    % test save
    savdir = 'D:\disc1_photometry_data\disc1_multi-fiber\Revision data\revision codes\np\reliability\CA1CA3\rand events\CA1CA3 test rand\Output';
    save(fullfile(savdir,[varName '_rand.mat']),'zscore_rand_CA1','zscore_rand_CA3') % save for individual animal
